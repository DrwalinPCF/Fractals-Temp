...
int main()
{
	...
	int i, j, k, l, beg;
	const int MEMORY_BLOCK_ELEMENTS = 1024*1024*64;
	const int ITERATIONS_QUEUEING = 1024*8;
	
	beg = clock();
	cl_kernel * function = CL::GetFunction( "Mandelbrot.c", "MandelbrotSet" );
	printf( "\n Compilation time: %ims", clock() - beg );
	
	beg = clock();
	
	std::vector<unsigned char>img( width*height*4 );
	
	unsigned char * img1 = CL::Allocate<unsigned char>( MEMORY_BLOCK_ELEMENTS*4, CL_MEM_WRITE_ONLY );//| CL_MEM_USE_HOST_PTR );
	
	float * ox = CL::Allocate<float>( 4, CL_MEM_READ_ONLY );
	float * oy = CL::Allocate<float>( 4, CL_MEM_READ_ONLY );
	...
	printf( "\n Allocation memory time: %ims", clock() - beg );
	...
	printf( "\n Calculated 100.0%% in: %ims", clock()-beg );
