...
int main()
{
	...
	unsigned char * img1 = CL::Allocate<unsigned char>( width*height*2/4, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR );
	unsigned char * img2 = CL::Allocate<unsigned char>( width*height*2/4, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR );
	unsigned char * img3 = CL::Allocate<unsigned char>( width*height*2/4, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR );
	unsigned char * img4 = CL::Allocate<unsigned char>( width*height*2/4, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR );
	int beg = clock();
	...
	for( i = 0; i < width*height; ++i )
	{
		...
		{
			v = img2[(i-(width*height/4))<<1];
			u = img2[((i-(width*height/4))<<1)+1];
		}
		else if( i < width*height*3/4 )
		{
			v = img3[(i-(width*height/2))<<1];
			u = img3[((i-(width*height/2))<<1)+1];
		}
		else if( i < width*height )
		{
			v = img4[(i-(width*height*3/4))<<1];
			u = img4[((i-(width*height*3/4))<<1)+1];
		}
		img_[(i<<2)+0] = v;
		img_[(i<<2)+1] = v;
		img_[(i<<2)+2] = u;
